<?php
/**
 * The template for displaying Comments
 *
 * The area of the page that contains comments and the comment form.
 *
 */

/*
 * If the current post is protected by a password and the visitor has not yet
 * entered the password we will return early without loading the comments.
 */
if ( post_password_required() ) {
	return;
}
?>

<div class="tab_5 desk_6 desk_large_8 margin_auto double_single_block comments" id="comments">
	<div class="padding_block">

		<?php if ( have_comments() ) : ?>
	
		<h2 class="single_block">
			<?php
				printf( _n( 'One commennt on &ldquo;%2$s&rdquo;', '%1$s comments on &ldquo;%2$s&rdquo;', get_comments_number(), 'jch' ),
					number_format_i18n( get_comments_number() ), get_the_title() );
			?>
		</h2>
	
		<ol class="comment_list">
			<?php
				wp_list_comments( array(
					'style'      => 'dl',
					'max_depth'  => 2,
					'callback' => 'advanced_comment',
					'short_ping' => true,
					'avatar_size'=> 64,
				) );
			?>
		</ol><!-- .comment-list -->
	
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
		<nav id="comment-nav-below" class="navigation comment-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php _e( 'Comment navigation', 'jch' ); ?></h1>
			<div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'jch' ) ); ?></div>
			<div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'jch' ) ); ?></div>
		</nav><!-- #comment-nav-below -->
		<?php endif; // Check for comment navigation. ?>
	
		<?php if ( ! comments_open() ) : ?>
		<p class="alt"><?php _e( 'Comments are closed.', 'jch' ); ?></p>
		<?php endif; ?>
	
		<?php endif; // have_comments() ?>
	
		<?php comment_form(array(
			'comment_notes_after' => '',
			)
		); ?>
	</div> <!-- padding_block -->
</div> <!-- tab_4 desk_6 desk_large_8 margin_auto single_block comments -->
